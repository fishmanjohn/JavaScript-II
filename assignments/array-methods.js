...
// Problem 1 pull runners id number and last name for "bib numbers"
let bibNumbers = []

runners.forEach(getBibs)
function getBibs(runners) {
  
  return bibNumbers.push(`${runners.id}, ${runners.last_name}`)
};
console.log(bibNumbers);

// Problem 2 pull companies for "thank you banner"
let thanksBanner = [];

  thanksBanner = runners.map((runners) =>{
  return `${runners.company_name}`;
})

console.log(thanksBanner);

// Problem 3 stagger the start list by size of runner filter for 3xl sized individuals to start at the front so all the runners finish arround the same time.

let clidesdaleClass = [];

clidesdaleClass = runners.filter((runners) =>{
return runners.shirt_size == '3XL';
})

console.log(clidesdaleClass);